...
from wordnet_demo import *
from baseline_stub_word2vec_demo import *
...
def find_answer(qgraph, sgraphs, qdiff, file):
    ...
    verb_ids = load_wordnet_verbs("Wordnet_verbs.csv")

    file = "-".join([file.split('-')[0], file.split('-')[1]])

    if qdiff == 'Hard':
        if qword != None:
            synonyms_of_qword = wn.synsets(qword)

            word = synonyms_of_qword[0]

            synonyms_of_qword += word.hyponyms()
            synonyms_of_qword += word.hypernyms()

            #print(synonyms_of_qword)

            for synset in synonyms_of_qword:
                # print(synset.name())
                if synset.name() in verb_ids[file]:
                    qset = set([verb_ids[file][synset.name()]])
            ...
            for node in sgraph.nodes.values():
                ...
                if node['head'] == snode["address"]:
                    ...
                    if qtype == 'Who':
                        ...
                        if snode['rel'] == "conj" or snode['rel'] == "ccomp" or snode['rel'] == "xcomp" or snode['rel'] == 'cc':
                            ...
                            # print(deps)
                            for dep in deps:
                                if dep['rel'] == 'nsubj' or dep['head']== snode['address']:
									 
                                    print('dep head: '+str(dep['head']))
                                    print('snode'+str(snode['address']))
                                    node = dep
                        #print(snode)
                        #print(node)
                    ...
                    if qtype == 'What':
                        ...
                        for dep in deps:
                            # print(dep['word'])
                            if dep['rel'] == 'xcomp' or dep['rel'] == 'dobj' or dep['rel'] == 'ccomp':
                                node = dep
                                deps = get_dependents(node, sgraph)
                                deps.append(node)
                                deps = sorted(deps, key=operator.itemgetter("address"))
