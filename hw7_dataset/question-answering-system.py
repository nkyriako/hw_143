...
from dependency_demo_new_stub import *
...
def baseline(qbow, sentences, isSch):
    ...
    if (qbow[0] == 'where'):
        searchResult = re.search(r'(?:.*) ((on|in|under|along) ((a|the) \w+ (of a \w+)?))',
                                 ' '.join(t[0] for t in best_answer))
        if searchResult:
    ...
    if (qbow[0] == 'why'):
        ...
        searchResult = re.findall(r'((because|since|for)(.+))(?: .?)', ' '.join(t[0] for t in best_answer))
        # re.findall(pattern, ' '.join(t[0] for t in best_answer)) #pattern.decode
        if searchResult:
            result = "".join([t for t in searchResult[0][0]])  # [0][-1]
...
def get_sentence_index(qbow, sentences):
    ...
    print("Sentence:"+ str(best_answer))
...
#     cname_size_dict.update({"fables": 1})
#     #cname_size_dict.update({"blogs": 2})
...
if __name__ == '__main__':
    ...
    for cname, size in cname_size_dict.items():
        for i in range(0, size):
            ...
            q_file = '{}.questions.dep'.format(fname)
            dep_file_sch = '{}.sch.dep'.format(fname)
            dep_file_story = '{}.story.dep'.format(fname)

            qgraphs = read_dep_parses(q_file)

            #reorder the qgraphs based on order of questions
            IDs = list()
            ...
            questions_dep = zip(IDs, qgraphs)

            questions_dep = sorted(questions_dep, key=lambda x:x[0])
            qgraphs = [qgraph for i, qgraph in questions_dep]



            # print(qgraphs[0])

            sgraphs = {'story': read_dep_parses(dep_file_story), 'sch':read_dep_parses(dep_file_sch)}

            for j, qgraph in enumerate(qgraphs):
                # print(qgraph)
                qname = "{0}-{1}".format(fname, j + 1)
                if qname in questions:
                    # print("QuestionID: " + qname)
                    ...
                    for qt in qtypes.split("|"):
                        ...
                        answer = find_answer(qgraph, sgraphs[qt])

                        if answer == None:
                            answer = baseline(qbow, sentences, True)
                            print("regex")

                        # if pretty_question(qgraph).split()[0] == "What":
                        print("Question:", pretty_question(qgraph), "?")
                        print("Answer:", answer)
                        print("Question Type: {}".format(qt))
                        print("Difficulty: {}".format(qdiff))
                        print()
